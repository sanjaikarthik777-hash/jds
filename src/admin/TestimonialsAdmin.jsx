...
import { getTestimonials, addTestimonial, updateTestimonial, deleteTestimonial } from '../store';
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            <div style={styles.modalHeader}>
              <h3 style={{ color: '#fff', margin: 0 }}>{editingId ? 'Edit Testimonial' : 'Add New Testimonial'}</h3>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                ...
                <div style={styles.inputGroup}>
                  ...
                  <input type="text" value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} style={styles.input} placeholder="e.g. Homeowner" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                <div style={styles.inputGroup}>
                  ...
                  <input type="number" min="1" max="5" value={formData.rating} onChange={e => setFormData({ ...formData, rating: Number(e.target.value) })} style={styles.input} />
                </div>
                <div style={styles.inputGroup}>
              ...
              <div style={styles.inputGroup}>
                ...
                <textarea value={formData.text} onChange={e => setFormData({ ...formData, text: e.target.value })} style={styles.textarea} rows={4} required />
              </div>
...
const styles = {
  ...
  card: { background: '#1a1a1f', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '1.5rem' },
  ...
  rating: { color: 'var(--accent-primary)', fontSize: '1rem', marginBottom: '1rem' },
  actions: { display: 'flex', gap: '0.5rem' },
  iconBtn: { background: 'rgba(255,255,255,0.05)', border: 'none', color: '#94a3b8', padding: '0.4rem', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' },
  cardDesc: { color: '#e2e8f0', margin: 0, fontSize: '0.95rem', lineHeight: '1.5', fontStyle: 'italic' },
  modalOverlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '1rem' },
  modal: { background: '#1a1a1f', width: '100%', maxWidth: '600px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' },
  ...
  textarea: { padding: '0.8rem', borderRadius: '6px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: '1rem', resize: 'vertical', outline: 'none' },
  saveBtn: { padding: '1rem', background: 'var(--accent-primary)', color: '#000', border: 'none', borderRadius: '6px', fontWeight: '600', fontSize: '1rem', cursor: 'pointer', marginTop: '1rem' }
};
